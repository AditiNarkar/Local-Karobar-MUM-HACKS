...
exports.uploadpart1 = async (req, res) => {
  ...
  try {
    ...
    let karobarExist = await USER.findOne({
      _id: user._id,
      "karobars.category": category,
      "karobars.orgEmail": orgEmail,
    });
...
exports.uploadpart2 = async (req, res) => {
  ...
  if (
    ...
    return res.json({
      status: 401,
      msg: "Please fill necessary (starred) fields.",
    });
  }
  ...
  if (user) {
    return res.status(400).json({
      msg: "Same Organization Name under same category cannot be formed, try adding branches to your already created karobar instead.",
    });
  }
  ...
  const newKarbar = {
    ...
    branchData: {
      brContact: brContact,
      brAddress: brAddress,
      lat: brLat, //optional feature, not compatible with current version
      long: brLng, //optional feature, not compatible with current version
      additionalInfo: {
        tag: addTag,
        description: addDesc,
      },
    ...
    stock: {
      stockQty: stockQty,
      stockMinLevel: stockMinLevel,
      stockMaxLevel: stockMaxLevel,
  ...
  return res.status(200).json({
    ...
    status: 200,
    branchIndex,
    msg: "Karobar Sucessfully Uploaded !!!",
  });
};
...
exports.insertPhotosInMongo = async (req, res) => {
  try {
    ...
    if (req.body.karobarIndex) {
      ...
      karobarIndex = userExist.karobars.findIndex((karobar) =>
        karobar._id.equals(req.body.karobarId)
      );
    }
    ...
    if (
      req.files.karobarphotos != null &&
      req.files.karobarphotos &&
      req.files.karobarphotos != " " &&
      req.files.karobarphotos != ""
    ) {
      let result;
    ...
    return res
      .status(200)
      .json({ status: 200, msg: "Karobar Sucessfully Uploaded !!!" });
  } catch (err) {
    console.log(err);
...
exports.forgotpassword = async (req, res) => {
  ...
  const result = await userExist.updateOne({
...
exports.viewKarobar = async (req, res) => {
  ...
  return res
    .status(200)
    .send({ karobar: karobarExist.karobars, user: userData });
};
...
exports.saveEditMyKarobar = async (req, res) => {
  ...
  try {
    ...
    const karobarIndex = userExist.karobars.findIndex((karobar) =>
      karobar._id.equals(karobarId)
    );
...
exports.getItemData = async () => {
  return await USER.aggregate([
    { $unwind: "$karobars" },
    { $unwind: "$karobars.items" },
    {
      $group: {
        _id: "$karobars.items.itemName",
        avgCostPrize: { $avg: "$karobars.items.itemCostPrize" },
        avgSellingPrize: { $avg: "$karobars.items.itemSellingPrize" },
      },
    },
  ]);
};
